...
/* Error messages */
static char AGMSG[] = "findrep: (sed) garbled address %s\n";
static char CGMSG[] = "findrep: (sed) garbled command %s\n";
static char TMTXT[] = "findrep: (sed) too much text: %s\n";
static char AD1NG[] = "findrep: (sed) no addresses allowed for %s\n";
static char AD2NG[] = "findrep: (sed) only one address allowed for %s\n";
static char TMCDS[] = "findrep: (sed) too many commands, last was %s\n";
static char COCFI[] = "findrep: (sed) cannot open command-file %s\n";
static char UFLAG[] = "findrep: (sed) unknown flag %c\n";
static char CCOFI[] = "findrep: (sed) cannot create %s\n";
static char ULABL[] = "findrep: (sed) undefined label %s\n";
static char TMLBR[] = "findrep: (sed) too many {'s\n";
static char FRENL[] = "findrep: (sed) first RE must be non-null\n";
static char NSCAX[] = "findrep: (sed) no such command as %s\n";
static char TMRBR[] = "findrep: (sed) too many }'s\n";
static char DLABL[] = "findrep: (sed) duplicate label %s\n";
static char TMLAB[] = "findrep: (sed) too many labels: %s\n";
static char TMWFI[] = "findrep: (sed) too many w files\n";
static char REITL[] = "findrep: (sed) RE too long: %s\n";
static char TMLNR[] = "findrep: (sed) too many line numbers\n";
static char TRAIL[] = "findrep: (sed) command \"%s\" has trailing garbage\n";
...
static char LTLMSG[] = "findrep: (sed) line too long\n";
...
/* Execute the compiled commands in cmds[] */
{
  ...
  for (;;) {
        ...
        if ((execp = getline_sed(linebuf)) == BAD) return;
        spend = execp;
        anysub = FALSE;
        ...
        if (!nflag && !delete) {
                for (p1 = linebuf; p1 < spend; p1++) putc(*p1, fpo);
                putc('\n', fpo);
        }
...
register char *ep;              /* regular expression element ptr */
{
  ...
  star:                         /* the recursion part of a * or + match */
            ...
            default:
                fprintf(stderr, "findrep: (sed) RE error, %o\n", *--ep);
                quit(2);
        }
}
...
sedcmd *ipc;
{
  ...
  switch (ipc->command) {
      case ACMD:                /* append */
        *aptr++ = ipc;
        if (aptr >= appends + MAXAPPENDS) fprintf(stderr,
                        "findrep: (sed) too many appends after line %ld\n",
                        lnum);
        *aptr = 0;
        break;
      ...
      case NCMD:                /* read next line into pattern space */
        ...
        if ((execp = getline_sed(linebuf)) == BAD) {
                delete = TRUE;
                break;
        }
        spend = execp;
      ...
      case CNCMD:               /* append next line to pattern space */
        if (aptr > appends) readout();
        *spend++ = '\n';
        if ((execp = getline_sed(spend)) == BAD) {
                *--spend = 0;
                break;
        }
        spend = execp;
...
cpcom:                          /* so s command can jump here */
      ...
      case RCMD:                /* read a file into the stream */
        *aptr++ = ipc;
        if (aptr >= appends + MAXAPPENDS) fprintf(stderr,
                        "findrep: (sed) too many reads after line %ld\n",
                        lnum);
        *aptr = 0;
        break;
